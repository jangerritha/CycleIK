...
from train_ik import train_ik, load_data
...
def train(trial):
    ...
    return train_ik(args, trial, config)

if __name__ == "__main__":
    study = None
    if args.db is not None:
        # You can change the number of GPUs per trial here:
        study = optuna.create_study(study_name="cycleik_ik_optimizer",
                                    direction='minimize',
    ...
    if args.add_seed:
        initial_layers = config["IKNet"]["architecture"]["layers"]
        initial_nbr_layers = len(initial_layers)

        initial_params ={"batch_size": config["IKNet"]["training"]["batch_size"],
         "lr": config["IKNet"]["training"]["lr"],
    ...
    joblib.dump(study, f"./optuna/{args.robot}/cycleik_ik_optimizer_{args.gpu}.pkl")

...
marker_publisher = rospy.Publisher("/visualization_marker", Marker, queue_size=1000000)
...
def main():
    ...
    with tqdm(total=iterations) as pbar:
        while correct_poses < iterations and not rospy.is_shutdown():
            ...
            random_js = [random.uniform(-1.329, 1.181),
                         random.uniform(-0.13, 0.666),
                         random.uniform(-0.23, 0.255),
                         random.uniform(-2.85, 2.0),
                         random.uniform(-1.266, 1.519),
                         random.uniform(-3.1, 2.18),
                         random.uniform(-0.12, 2.174),
                         random.uniform(-2.019, 3.14),
                         random.uniform(-0.625, 0.62),
                         random.uniform(-0.49, 0.36)]

            # rospy.loginfo("Position: \n%s", correct_poses)
            # rospy.loginfo("Joint State: \n%s", random_js)

            joint_state_msg = JointState()
            joint_state_msg.name = ["torsoYaw", "torsoPitch", "torsoRoll", "rightShoulderPitch", "rightShoulderRoll", "rightShoulderYaw", "rightElbowPitch", "rightForearmYaw", "rightWristRoll", "rightWristPitch"]
            ...
            result = None
            try:
                result = calculate_fk(h, ["rightPalm"], robot_state_msg)
    ...
    with open('./data_samplers/results_valkyrie_200_1.p', 'wb') as f:
        pickle.dump(data, f)
        f.close()
